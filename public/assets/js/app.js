(() => {
  ...
  const DEFAULT_USERS = [
    { email: "superadmin@test.com", password: "123", role: "superadmin", name: "ICT Coordinator" },
    { email: "admin@test.com", password: "123", role: "admin", name: "School Principal" },
  ...
  const ROLE_META = {
    ...
    superadmin: { label: "ICT Coordinator", color: "#1e3a5f", initials: "S" },
    admin: { label: "School Principal", color: "#0d9488", initials: "A" },
    custodian: { label: "Property Custodian", color: "#14b8a6", initials: "C" },
    teacher: { label: "Teacher", color: "#2dd4bf", initials: "T" },
    // Legacy read-only principal — retired into 'admin' at the DB level,
    // kept here only so any stale session string still renders safely.
    principal: { label: "School Principal", color: "#0d9488", initials: "A" }
  };
  ...
  const NAVIGATION = {
    // ── SUPER ADMIN (ICT Coordinator) — technical / maintenance ──
    superadmin: [
      ...
      { id: "resetRequests", label: "Password Resets", href: "users.html#reset-requests-card", icon: "key-round" },
      { id: "maintenance", label: "System Maintenance", href: "policy.html#maintenance", icon: "settings" },
      { id: "account", label: "Account Settings", href: "account.html", icon: "user" }
    ],
    // ── ADMIN (School Principal) — analytics, approvals, oversight ──
    admin: [
      { id: "dashboard", label: "Dashboard", href: "dashboard.html", icon: "layout-dashboard" },
      { id: "inventory", label: "Inventory (View)", href: "inventory.html", icon: "package" },
      { id: "reports", label: "Reports & Analytics", href: "reports.html", icon: "bar-chart-3" },
      { id: "transactions", label: "Requests & Logs", href: "transactions.html", icon: "scroll-text" },
      { id: "activity", label: "Activity Log", href: "reports.html#activity", icon: "history" },
      { id: "account", label: "Account Settings", href: "account.html", icon: "user" }
    ],
    // ── CUSTODIAN (Property Custodian) — main operator ──
    custodian: [
      ...
      { id: "policy", label: "Lending Policy", href: "policy.html", icon: "clipboard-list" },
      { id: "account", label: "Account Settings", href: "account.html", icon: "user" }
    ],
    // ── TEACHER — borrow / return / view ──
    teacher: [
      { id: "dashboard", label: "Dashboard", href: "dashboard.html", icon: "layout-dashboard" },
  ...
  const PAGE_PERMISSIONS = {
    dashboard:    ["superadmin", "admin", "custodian", "teacher"],
    inventory:    ["superadmin", "admin", "custodian"],
    transactions: ["superadmin", "admin", "custodian", "teacher"],
    borrow:       ["teacher"],
    return:       ["teacher"],
    reports:      ["superadmin", "admin", "custodian"],
    users:        ["superadmin", "admin"],
    policy:       ["superadmin", "admin", "custodian"],
    account:      ["superadmin", "admin", "custodian", "teacher"],
  };
  ...
  function normalizeRole(role) {
    return ["superadmin", "admin", "custodian", "teacher", "principal"].includes(role) ? role : "teacher";
  }
  ...
  function updateUser(email, payload) {
    ...
    const superCount = users.filter((user) => user.role === "superadmin").length;
    if (current.role === "superadmin" && nextRole !== "superadmin" && superCount <= 1) {
      return { ok: false, error: "At least one ICT Coordinator (Super Admin) account is required." };
    }
    if (current.role === "admin" && nextRole !== "admin" && adminCount <= 1) {
      return { ok: false, error: "At least one Principal (Admin) account is required." };
    }
  ...
  function deleteUser(email) {
    ...
    const adminCount = users.filter((user) => user.role === "admin").length;
    const superCount = users.filter((user) => user.role === "superadmin").length;
    if (userToDelete.role === "superadmin" && superCount <= 1) {
      return { ok: false, error: "At least one ICT Coordinator (Super Admin) account is required." };
    }
  ...
  // v10.5 — Email-free mode. The PHP backend still stores an email
  // column, so we synthesize one from the username for new accounts.
  // The "@local" suffix is a private convention so we can tell
  // synthesized emails apart from any real ones still in the DB.
  function usernameToEmail(username) {
    const u = String(username || "").trim();
    if (!u) return "";
    // If the user typed something that already looks like an email
    // (e.g. an existing admin recovering an old account) keep it.
    if (/^[^\s@]+@[^\s@]+\.[^\s@]+$/.test(u)) return u.toLowerCase();
    // v10.8 — the previous synthetic suffix "@local" doesn't pass
    // PHP's FILTER_VALIDATE_EMAIL (needs a TLD after the dot), so
    // the User Management form choked with "Please enter a valid
    // email address" even though the field is hidden. Use a proper
    // synthetic domain that validates as an RFC-compliant email.
    // The "@platonian.local" suffix is still recognizable as a
    // synthesized address so we can distinguish it from real ones.
    const safe = u.replace(/[\s@]/g, "_");
    return `${safe}@platonian.local`.toLowerCase();
  }
  function isLikeEmail(value) {
  ...
  // ── SERVER-SOURCED CACHE (v9) ─────────────────────────────
  // The dashboard reads assets/transactions/borrowRequests through
  // getAssets() / getTransactions() / getBorrowRequests(). Pre-v9
  // those read from localStorage and never reflected the real MySQL
  // state. The patching IIFE below now fetches fresh data from the
  // API on dashboard load and stores it here. These getters check
  // the cache first and fall back to localStorage so other modules
  // (inventory/transactions/etc) keep working before sync completes.
  const _serverCache = {
    assets:         null, // null = unset, [] = empty result
    transactions:   null,
    borrowRequests: null,
  };

  function setServerCache(partial) {
    if (!partial || typeof partial !== "object") return;
    if (Array.isArray(partial.assets))         _serverCache.assets         = partial.assets;
    if (Array.isArray(partial.transactions))   _serverCache.transactions   = partial.transactions;
    if (Array.isArray(partial.borrowRequests)) _serverCache.borrowRequests = partial.borrowRequests;
  }

  function getAssets() {
    if (_serverCache.assets !== null) return _serverCache.assets;
  ...
  function getTransactions() {
    const source = _serverCache.transactions !== null
      ? _serverCache.transactions
      : readArray(STORAGE_KEYS.transactions);
  ...
  function getDashboardStats(user = null) {
    ...
    // Needs Repair is derived from BOTH the live asset records and the
    // transaction history so it always matches the real data:
    //   1. Assets explicitly flagged "Under Repair" (status), OR whose
    //      stored condition is a damaged/repair condition.
    //   2. Units that were returned in a damaged condition (the asset may
    //      have already been restocked, so the transaction is the source).
    // Assets counted via (1) are tracked by id so the matching returned
    // transactions in (2) are not double counted.
    const repairAssetIds = new Set();
    const needsRepairFromAssets = assets.reduce((sum, asset) => {
      const status = String(asset.status || "").trim().toLowerCase();
      const condition = String(asset.condition || "").trim().toLowerCase();
      const flagged = status === "under repair" || isDamagedCondition(condition);
      if (!flagged) return sum;
      repairAssetIds.add(String(asset.id || ""));
      return sum + Math.max(1, Number(asset.quantity) || 0);
    }, 0);

    const needsRepairFromReturns = getTransactions().reduce((sum, transaction) => {
      if (transaction.status !== "Returned") return sum;
      if (!isDamagedCondition(transaction.returnedCondition)) return sum;
      // Skip if the asset itself is already counted as under repair above.
      if (repairAssetIds.has(String(transaction.assetId || ""))) return sum;
      return sum + (Number(transaction.quantity) || 1);
    }, 0);

    const needsRepair = needsRepairFromAssets + needsRepairFromReturns;
  ...
  // Shared helper: which condition strings mean an item is damaged / needs
  // repair. Used by the dashboard stats and the condition breakdown so the
  // "Needs Repair" metric is computed consistently from the real data.
  function isDamagedCondition(value) {
    const v = String(value || "").trim().toLowerCase();
    if (!v) return false;
    return v === "needs repair"
      || v === "for repair"
      || v === "damaged"
      || v === "broken"
      || v === "defective"
  ...
  function getConditionBreakdown(user = null) {
    ...
    // Admin / custodian view: build the condition breakdown straight from the
    // real asset records using each asset's actual `condition` value. Assets
    // flagged "Under Repair" are surfaced as "Needs Repair" regardless of the
    // stored condition string so the chart matches the dashboard stat.
    const bucket = new Map();
    getAssets().forEach((asset) => {
      const status = String(asset.status || "").trim().toLowerCase();
      const qty = Math.max(1, Number(asset.quantity) || 0);
      let label = String(asset.condition || "").trim() || "Good";
      if (status === "under repair" || isDamagedCondition(label)) {
        label = "Needs Repair";
      }
    ...
    const entries = Array.from(bucket.entries())
      ...
      .map(([label, value]) => ({ label, value }))
      .sort((a, b) => b.value - a.value);
    return entries.length ? entries : [{ label: "Good", value: 1 }];
  }
  ...
  function icon(name) {
    const icons = {
      ...
      // v10.5 — Account Settings nav icon (was missing, hence the empty space)
      ...
      // v10.17 — previously-missing icons that rendered as empty space:
      // Lending Policy (custodian), Password Resets + System Maintenance
      // (superadmin), Reports & Analytics + Activity Log (admin).
      "clipboard-list": '<svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><rect width="8" height="4" x="8" y="2" rx="1" ry="1"></rect><path d="M16 4h2a2 2 0 0 1 2 2v14a2 2 0 0 1-2 2H6a2 2 0 0 1-2-2V6a2 2 0 0 1 2-2h2"></path><path d="M12 11h4"></path><path d="M12 16h4"></path><path d="M8 11h.01"></path><path d="M8 16h.01"></path></svg>',
      "key-round": '<svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="M2 18v3c0 .6.4 1 1 1h4v-3h3v-3h2l1.4-1.4a6.5 6.5 0 1 0-4-4Z"></path><circle cx="16.5" cy="7.5" r=".5" fill="currentColor"></circle></svg>',
      ...
      "bar-chart-3": '<svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="M3 3v18h18"></path><path d="M18 17V9"></path><path d="M13 17V5"></path><path d="M8 17v-3"></path></svg>',
      history: '<svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="M3 12a9 9 0 1 0 9-9 9.75 9.75 0 0 0-6.74 2.74L3 8"></path><path d="M3 3v5h5"></path><path d="M12 7v5l4 2"></path></svg>'
    };
    return icons[name] || "";
  }
  ...
  function renderSidebar(pageKey, user) {
    ...
    // v10.5 — render a bottom navigation bar for mobile. Use the same
    // NAVIGATION config so the two stay in sync. We keep the bar to a
    // sensible number of items by showing the first 5 nav entries and,
    // when there are more, putting the rest behind a "More" tap that
    // opens the off-canvas sidebar. The bar is added next to <main> so
    // it floats above the content and is hidden by CSS on >760px.
    renderBottomNav(navItems, pageFile, pageHash, pageKey);
  }

  // v10.5 — Mobile bottom navigation
  function renderBottomNav(navItems, pageFile, pageHash, pageKey) {
    ...
    if (!bar) {
      ...
      // Insert before any footer if present, or at body end
      document.body.appendChild(bar);
    }
    const inner = bar.querySelector(".bottom-nav-inner");
    if (!inner) return;

    // Decide which items go on the bar. We prefer to show the most
    // important pages first; the rest fall under a "More" button that
    // opens the off-canvas sidebar.
    const PRIMARY_LIMIT = 5;
    const primary = navItems.slice(0, PRIMARY_LIMIT);
    const overflow = navItems.slice(PRIMARY_LIMIT);

    const items = primary.map((item) => {
      const activeClass = isNavItemActive(item, navItems, pageFile, pageHash, pageKey) ? "active" : "";
      return `<a class="bottom-nav-item ${activeClass}" href="${item.href}">
        <span class="bn-icon">${icon(item.icon) || icon("layout-dashboard")}</span>
        <span class="bn-label">${escapeHtml(shortenLabel(item.label))}</span>
      </a>`;
    });

    if (overflow.length) {
      items.push(`<button type="button" class="bottom-nav-item" id="bottom-nav-more" aria-label="More">
        <span class="bn-icon">${icon("scroll-text")}</span>
  ...
  function shortenLabel(label) {
    ...
    const map = {
      ...
      "View Borrowed Assets": "Borrowed",
      "Lending Policy": "Policy",
      "Users (View)": "Users",
      "Borrow Item": "Borrow",
      "Return Item": "Return"
    };
  ...
  function applyManagementControlVisibility(user) {
    const role = normalizeRole(user?.role);
    // v10.16 four-role model. "Management controls" = inventory write
    // controls (add / edit / remove assets, registration). These belong
    // to the Custodian (main operator) and the ICT Super Admin only.
    //   - teacher  : hide (borrow/return only)
    //   - admin    : hide inventory writes (Principal is oversight +
    //                approvals; approve/reject buttons are gated elsewhere)
    //   - principal: legacy alias of admin — hide
    const shouldHide = role === "teacher" || role === "admin" || role === "principal";
    const controls = document.querySelectorAll("[data-management-control='true']");
    controls.forEach((control) => {
      control.hidden = shouldHide;
    });
    // Hard-disable write forms for the read-oversight roles so they can't
    // bypass the hidden style via devtools. Approve/reject controls are NOT
    // marked data-write-form, so the Principal keeps those.
    if (role === "admin" || role === "principal" || role === "teacher") {
      const writeForms = document.querySelectorAll("form[data-write-form='true']");
      writeForms.forEach((form) => {
        form.querySelectorAll("input, select, textarea, button").forEach((el) => {
          el.disabled = true;
        });
        // Visual hint
        if (!form.classList.contains("readonly-form")) form.classList.add("readonly-form");
      });
    }
  }
  ...
  function restoreLegacyLogos() {
    // v10.4 fix — older versions of this helper hard-coded src="logo.png"
    // which resolved against the page directory and broke whenever the
    // HTML actually wanted "assets/img/logo.png". Only fix up missing
    // src attributes and never overwrite a perfectly good path the HTML
    // already provided.
    const logos = document.querySelectorAll("img.site-logo-img");
    logos.forEach((logo) => {
      const existing = logo.getAttribute("src");
      if (!existing) logo.setAttribute("src", "assets/img/logo.png");
  ...
  function initLayout(pageKey, pageTitle) {
    ...
    if (logoutButton) {
      logoutButton.addEventListener("click", async () => {
        // v10: call server-side logout to destroy the PHP session
        // and invalidate the CSRF token. Even if the network fails,
        // we still clear the client state and redirect.
        try {
  ...
  // Stable color per condition label, with a fallback palette for any
  // extra labels. "Needs Repair" is always red so it stands out.
  function conditionColorFor(label, fallbackIndex) {
    const map = {
      "excellent": "#16a34a",
      "very good": "#22c55e",
      "new": "#10b981",
    ...
    const palette = ["#3b82f6", "#f59e0b", "#8b5cf6", "#14b8a6", "#ec4899", "#64748b"];
  ...
  function renderConditionChart(canvasId, user = null) {
    ...
    const colors = entries.map((entry, index) => conditionColorFor(entry.label, index));
    chartRegistry[canvasId] = new Chart(canvas, {
      type: "doughnut",
      data: {
  ...
  function masterInventoryRowsFromAssets(assets = getAssets()) {
    ...
    return safeAssets.map((asset) => {
      ...
      // v10.22 — an item can hold multiple attachments now.
      const files = Array.isArray(asset?.receiptFiles) && asset.receiptFiles.length
        ? asset.receiptFiles
        : (asset?.receiptFile ? [asset.receiptFile] : []);
      return {
        ...
        "IAR Files": files.length ? files.join("; ") : "Not uploaded",
  ...
  function exportMasterInventoryCsv(fileName = `master_inventory_${todayISO()}.csv`) {
    const headers = ["Asset ID", "Name", "Description", "Category", "DepEd / Bulk Item", "Bulk ID", "Quantity", "Location", "Status", "Unit Cost (PHP)", "IAR Files", "Date Added"];
    ...
    if (user.role === "teacher") {
      ...
      if (tierData && tierData.tier >= 1) {
        ...
        banner.innerHTML = `
          ...
          <div>
            <div>${escapeHtml(tierData.message || "")}</div>
  ...
  function getBorrowRequests() {
    if (_serverCache.borrowRequests !== null) return _serverCache.borrowRequests;
  ...
  window.IMS = {
    ...
    // v10.17 — let pages re-sync the highlighted nav item when the
    // hash changes (e.g. Inventory Management <-> Asset Registration,
    // which live on the same inventory.html page behind #register).
    refreshNavActive: function () {
      const user = (window.API && window.API.getSessionUser && window.API.getSessionUser())
        || getCurrentUser();
      if (!user) return;
      let pageKey = (document.body && document.body.dataset.page) || "";
      // The hash maps to a distinct nav entry (e.g. #register ->
      // Asset Registration). Resolve the pageKey to that nav id so the
      // correct item lights up.
      const navItems = NAVIGATION[normalizeRole(user.role)] || [];
      const pageFile = window.location.pathname.split("/").pop() || "dashboard.html";
      const pageHash = window.location.hash || "";
      const hashMatch = navItems.find((it) => it.href === pageFile + pageHash);
      if (hashMatch) pageKey = hashMatch.id;
      renderSidebar(pageKey, { ...user, role: normalizeRole(user.role) });
    },
    getUsers,
    ...
    // v10.9: expose the dashboard-scoped txn list so the
    // Monthly Activity range filter (v10.9.js) can rebuild
    // the chart for any time bucket (daily/weekly/quarterly/yearly).
    getDashboardTransactions: () => {
      const u = (window.API && window.API.getSessionUser && window.API.getSessionUser()) || null;
      return getTransactionsForDashboard(u);
    },
    createCategoryBadge,
    createStatusBadge,
    ...
    setServerCache,
    initDashboardPage,
    // v10.5 — synthetic email derived from a username so the existing
    // PHP backend (which still requires an email field) keeps working
    // even though the UI no longer asks for one.
    usernameToEmail,
    isLikeEmail
  };

  // ============================================================
  // SECTION 2: notifications + condition autofill + auth-session patch
  // (was a separate IIFE pre-v10; merged for single-scope clarity)
  // ============================================================
  ...
  // ── SERVER SYNC (v9 carried into v10) ───────────────────────
  // The dashboard's KPIs, charts, low-stock list, and recent
  // activity panel all read through window.IMS.getAssets()/
  // getTransactions()/getBorrowRequests(). Pre-v9 those read
  // from localStorage and never reflected the real MySQL state.
  // syncFromServer() fetches the three list endpoints in
  // parallel and hydrates the cache via setServerCache().
  async function syncFromServer() {
    if (!window.API) return false;
    try {
      const [assets, transactions, borrowRequests] = await Promise.all([
        window.API.getAssets(),
        window.API.getTransactions(),
        window.API.getBorrowRequests(),
      ]);
      window.IMS.setServerCache({ assets, transactions, borrowRequests });
      return true;
    } catch (err) {
      console.warn("[v10 dashboard] sync from server failed, falling back to local cache:", err);
      return false;
    }
  }
  window.IMS.syncFromServer = syncFromServer;

  // Wrap initDashboardPage to await a fresh sync first. Without
  // this, the dashboard renders stale localStorage seed data.
  const _origInitDashboardPage = window.IMS.initDashboardPage;
  window.IMS.initDashboardPage = async function() {
    const loadingHint = document.getElementById("dashboard-sync-hint");
    if (loadingHint) loadingHint.style.display = "inline-flex";
    await syncFromServer();
    if (loadingHint) loadingHint.style.display = "none";
    return _origInitDashboardPage();
  };
  ...
  function renderNotifList(notifications) {
    ...
    list.querySelectorAll(".notif-item").forEach(item => {
      item.addEventListener("click", async () => {
        ...
        const n = _lastNotifs.find(x => x.notifId === nid);
        if (n && n.link) window.location.href = n.link;
        // Recompute unread count AND whether overdue items still remain unread,
        // so the bell stays red/shaking while real overdue items linger.
        const stillUnread = _lastNotifs.filter(x => !x.isRead && x.notifId !== nid);
        const stillHasOverdue = stillUnread.some(x => x.type === 'overdue');
        updateBadge(stillUnread.length, stillHasOverdue);
      });
    });
  }
  ...
  const escHtml = escapeHtml; // alias — avoids a second implementation
  ...
  function initNotificationBell() {
    ...
    if (markAllBtn) {
      markAllBtn.addEventListener("click", async e => {
        ...
        lastUnreadCount = 0;
        _lastNotifs = _lastNotifs.map(n => ({ ...n, isRead: true }));
      });
    }
  ...
  async function renderBackupHistory() {
    ...
    container.innerHTML = backups.map(b => {
      ...
      const fn = escHtml(b.filename);
      const dt = escHtml(b.date);
      return `<div style="display:flex; align-items:center; justify-content:space-between; padding:0.55rem 0; border-bottom:1px solid var(--gray-100); font-size:0.82rem;">
        <div>
          <span style="font-family:'DM Mono',monospace; color:var(--gray-700);">${fn}</span>
          <span style="color:var(--gray-400); margin-left:0.5rem;">${dt} · ${kb} KB</span>
        </div>
        <button class="btn btn-secondary btn-sm" data-backup-download="${fn}" type="button">↓ Download</button>
      </div>`;
    }).join('');
    // Wire download buttons via delegation (safer than inline onclick + interpolation)
    container.querySelectorAll('[data-backup-download]').forEach(btn => {
      btn.addEventListener('click', () => {
        const f = btn.getAttribute('data-backup-download');
        if (f) window.API.downloadBackup(f);
      });
    });
  }
