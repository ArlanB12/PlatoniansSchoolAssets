...
(() => {
  ...
  // ── NAME AUTOCOMPLETE ─────────────────────────────────────
  // Maps a raw DB role to one of the values the checkout role <select>
  // actually offers, so auto-detect always lands on a real option.
  function normalizeBorrowerRole(raw) {
    const r = String(raw || "").trim().toLowerCase();
    if (r === "admin" || r === "superadmin") return "admin";
    if (r === "custodian") return "custodian";
    // teacher, principal, or anything unknown → treated as teacher
    return "teacher";
  }

  // Auto-fills the role <select> from the picked borrower's account role and
  // locks it so it can't be changed by hand. If the field is left empty / typed
  // manually, the lock is released so the staff can still set it themselves.
  function applyAutoRole(roleSelectId, role) {
    const sel = document.getElementById(roleSelectId);
    if (!sel) return;
    const val = normalizeBorrowerRole(role);
    sel.value = val;
    sel.dataset.autoRole = "1";
    sel.disabled = true;                 // visually locked
    sel.style.background = "#f0fdf4";
    sel.style.color = "#166534";
    sel.style.fontWeight = "600";
    sel.title = "Auto-detected from the borrower's account";
    // A disabled <select> is NOT submitted with the form, but we read role
    // straight from the element on submit so this is fine.
    let hint = document.getElementById(roleSelectId + "-autohint");
    if (!hint) {
      hint = document.createElement("div");
      hint.id = roleSelectId + "-autohint";
      hint.className = "hint-text";
      hint.style.color = "#0f766e";
      sel.parentNode.appendChild(hint);
    }
    hint.textContent = "Auto-detected from the borrower's account record.";
  }

  function clearAutoRole(roleSelectId) {
    const sel = document.getElementById(roleSelectId);
    if (!sel) return;
    delete sel.dataset.autoRole;
    sel.disabled = false;
    sel.style.background = "";
    sel.style.color = "";
    sel.style.fontWeight = "";
    sel.title = "";
    const hint = document.getElementById(roleSelectId + "-autohint");
    if (hint) hint.textContent = "Type a known borrower to auto-detect their role.";
  }

  function wireNameAutocomplete(inputId, hiddenEmailId, roleSelectId) {
    ...
    inp.addEventListener("input", () => {
      ...
      // The moment the name is edited by hand, the previously auto-detected
      // role no longer applies — unlock it so staff can pick manually.
      if (roleSelectId) clearAutoRole(roleSelectId);
      ...
      timer = setTimeout(async () => {
        ...
        box.innerHTML = sugs.map(s =>
          `<div class="suggest-item" data-name="${window.IMS.escapeHtml(s.name)}" data-email="${window.IMS.escapeHtml(s.email)}" data-role="${window.IMS.escapeHtml(s.role)}">
             <span class="suggest-name">${window.IMS.escapeHtml(s.name)}</span>
             <span class="suggest-meta">${window.IMS.escapeHtml(window.IMS.roleLabel ? window.IMS.roleLabel(s.role) : s.role)} · ${window.IMS.escapeHtml(s.email)}</span>
           </div>`
        ).join("");
        ...
        box.querySelectorAll(".suggest-item").forEach(item => {
          item.addEventListener("click", () => {
            ...
            const hiddenEl2 = document.getElementById(hiddenEmailId);
            if (hiddenEl2) hiddenEl2.value = item.getAttribute("data-email");
            // ── AUTO-DETECT ROLE from the chosen borrower's account ──
            if (roleSelectId) applyAutoRole(roleSelectId, item.getAttribute("data-role"));
  ...
  async function renderPendingRequests(user) {
    ...
    body.innerHTML = [...requests].reverse().map(r => {
      ...
      return `<tr>
        ...
        <td class="action-cell"><div class="action-cell-inner">${actions}</div></td>
      </tr>`;
    }).join("");

    // v10.20: the approve / reject ACTION (confirm dialog + API call +
    // refresh) is now owned entirely by the delegated handler in
    // v10.20.js. Keeping a second onclick worker here caused the
    // confirm dialog to fire in an endless loop (the old confirm code
    // replayed btn.click(), which this handler then re-processed). We
    // expose a refresh hook instead so v10.20.js can repaint after a
    // successful action, and we deliberately do NOT bind body.onclick.
    body.onclick = null;
  }

  // Allow the centralized confirm handler (v10.20.js) to refresh the
  // page after it approves / rejects a request.
  if (window.IMS) {
    window.IMS.__refreshTransactionsPage = (u) => refreshTransactionsPage(u);
    window.IMS.__renderPendingRequests   = (u) => renderPendingRequests(u);
  }
  ...
  async function initTransactionsPage() {
    ...
    if (!user) return;
    // v10.16: only the Custodian performs manual check-in / check-out.
    // The Principal (admin) and ICT (superadmin) get a read-only logs view
    // plus the approve/reject queue (admin only acts on it).
    const isCustodian = user.role === "custodian";
    const title = user.role === "teacher"
      ? "View Borrowed Assets"
      : isCustodian
        ? "Check In / Check Out"
        : "Requests & Transaction Logs";
    window.IMS.initLayout("transactions", title);

    // Hide the manual checkout/return sections for everyone except the
    // Custodian (the operator). Teachers, Principal, and ICT never see them.
    if (!isCustodian) {
    ...
    if (checkoutForm && isCustodian) {
      ...
      // Borrower name autocomplete — also auto-detects the borrower's role.
      wireNameAutocomplete("checkout-borrower-name", "checkout-borrower-email-hidden", "checkout-role");
      // Start with a small hint under the role field.
      clearAutoRole("checkout-role");
      ...
      checkoutForm.addEventListener("submit", async (e) => {
        ...
        // Empty-field check must run BEFORE date range checks — otherwise
        // an empty date string compares as less than today and shows a
        // misleading "past date" error when the real problem is a blank field.
  ...
  async function renderMyRequests(user) {
    ...
    if (!mine.length) { body.innerHTML = '<tr><td colspan="7">No borrow requests yet.</td></tr>'; return; }
    const statusColors = { Pending:"#b45309", Approved:"#166534", Rejected:"#991b1b" };
    const statusBg     = { Pending:"#fef3c7", Approved:"#dcfce7", Rejected:"#fee2e2" };
  ...
  async function initBorrowPage() {
    ...
    if (form) {
      form.addEventListener("submit", async (e) => {
        ...
        // Empty-field check must come first (see checkout form for rationale).
        if (!assetId || !borrower || !borrowDate || !returnDate) {
        ...
        // Server pulls borrower / borrowerEmail / role from the session now,
        // so we only need to send the request details.
        const res = await window.API.createBorrowRequest({ assetId, quantity, borrowDate, returnDate, condition });
        if (!res.ok) {
  ...
  async function initReportsPage() {
    ...
    if (runNowBtn) {
      runNowBtn.addEventListener("click", async () => {
        ...
        if (res.ok) {
          ...
          if (mod && window.API.listBackups) {
            mod.innerHTML = '<div style="font-size:0.82rem;color:var(--gray-500);">Refreshing…</div>';
            setTimeout(async () => {
              const backups = await window.API.listBackups();
              if (!backups.length) {
                mod.innerHTML = '<div style="font-size:0.82rem;color:var(--gray-500);">No backups found.</div>';
                return;
              }
              mod.innerHTML = backups.map(b => {
                const kb = (b.size / 1024).toFixed(1);
                const fn = esc(b.filename);
                const dt = esc(b.date);
                return `<div style="display:flex;align-items:center;justify-content:space-between;padding:0.55rem 0;border-bottom:1px solid var(--gray-100);font-size:0.82rem;">
                  <div><span style="font-family:'DM Mono',monospace;color:var(--gray-700);">${fn}</span><span style="color:var(--gray-400);margin-left:0.5rem;">${dt} · ${kb} KB</span></div>
                  <button class="btn btn-secondary btn-sm" data-backup-dl="${fn}" type="button">↓ Download</button>
                </div>`;
              }).join('');
              mod.querySelectorAll('[data-backup-dl]').forEach(btn => {
                btn.addEventListener('click', () => {
                  const f = btn.getAttribute('data-backup-dl');
    ...
    if (backupBtn && user.role === "superadmin") {
    ...
    if (restoreInput && user.role === "superadmin") {
  ...
  document.addEventListener("DOMContentLoaded", async () => {
    ...
    // v10: ensure server session + CSRF token are loaded before
    // any of the page initializers runs requireAuth/API calls.
