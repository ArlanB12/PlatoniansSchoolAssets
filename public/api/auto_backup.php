...
function buildBackupData($db) {
    ...
    // original_password was removed in v8.1 — only password (bcrypt hash) is stored
    $users = $db->query("SELECT email, password, role, name FROM users ORDER BY id")->fetchAll();
    ...
    $normalizeAsset = fn($r) => [
        ...
        'receiptFiles' => (function($v){ $d = json_decode($v ?? '[]', true); return is_array($d) ? $d : []; })($r['receipt_files'] ?? '[]'),
